...
const BAKE_PATH = '/portfolio-decor-bake';
const BAKE_SELECTOR = '.portfolio-decor-bake-root';

const scriptDir = path.dirname(fileURLToPath(import.meta.url));
const projectRoot = path.join(scriptDir, '..');
const outputDir = path.join(projectRoot, 'public', 'portfolio', 'optimized');
...
async function bakeDecorStack() {
  ...
  try {
    ...
    const page = await browser.newPage({
      viewport: { width: 1440, height: 2737 },
      deviceScaleFactor: 1,
    });
