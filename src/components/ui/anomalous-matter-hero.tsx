...
export function GenerativeArtScene({
...
}: GenerativeArtSceneProps): React.JSX.Element {
  ...
  useEffect(() => {
    ...
    const geometry = new THREE.IcosahedronGeometry(geometryRadius, GEOMETRY_DETAIL);
    const material = new THREE.ShaderMaterial({
      ...
      vertexShader: `
        ...
        void main() {
          vNormal = normal;
          vPosition = position;
          float displacement = snoise(position * 2.0 + time * 0.5) * ${DISPLACEMENT_SCALE.toFixed(1)};
    ...
    const animate = (timestamp: number): void => {
      material.uniforms.time.value = timestamp * TIME_SCALE;
    ...
    return () => {
      ...
      disposeSceneResources(geometry, material, renderer);
    };
  }, [cameraDistance, color, geometryRadius]);
...
}: AnomalousMatterHeroProps): React.JSX.Element {
  return (
    <section
      role="banner"
    ...
    >
      ...
      <div className="relative z-20 flex h-full flex-col items-center justify-end pb-20 text-center md:pb-32">
        <div className="max-w-3xl px-4">
          <h1 className="font-mono text-sm uppercase tracking-widest text-[var(--home-accent)]/80">
            {title}
          </h1>
